...
{
    global $db;
    $query = $db->listgo("`id`,`COL_Iname`,`COL_Iintegral`,`COL_Iconfirm`,`COL_ITime`", "`emmm_integral`", "where `COL_Iuseremail` = '" . $_SESSION['username'] . "' order by id desc");
    $userrows = array();
    while ($emmm_rs = $db->whilego($query)) {
        $userrows[] = array(
            'id' => $emmm_rs[0],
...
$smarty->assign('user', emmm_userview());
